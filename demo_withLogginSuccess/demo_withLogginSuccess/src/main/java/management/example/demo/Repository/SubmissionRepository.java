...
public interface SubmissionRepository extends JpaRepository<Submission, Long> {
    ...
    @Query(value = "SELECT cs.reg_number AS regNumber, cs.registration_number AS registrationNumber, cs.name_with_initials AS nameWithInitials, " +
            "s.title AS title, s.deadline AS deadline, s.submission_status AS submissionStatus, s.deadline_to_review AS deadlineToReview, " + // Added space after deadlineToReview
            ...
            "GROUP BY cs.reg_number, cs.registration_number, cs.name_with_initials, s.title, s.deadline, s.submission_status, s.deadline_to_review",
            nativeQuery = true)
